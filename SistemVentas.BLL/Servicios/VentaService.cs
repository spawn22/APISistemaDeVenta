        ...
        public async Task<List<VentaDTO>> Historial(string buscarPor, string numeroVenta, string fechaInicio, string fechaFin)
        {
            ...
            try
            {
                ...
                if(buscarPor == "fecha")
                {
                    ...
                    v.FechaRegistro.Value.Date <= fech_Fin.Date).Include(detallev => detallev.DetalleVenta )
                ...
                {
                    ListaResultado = await query.Where(v => v.NumeroDocumento == numeroVenta).Include(detallev => detallev.DetalleVenta)
                    .ThenInclude(p => p.IdProductoNavigation).ToListAsync();
                }
